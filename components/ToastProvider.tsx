...
// Small pill toasts at top-centre, newest on top, auto-dismiss after 3s.
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Ctx.Provider value={{ push }}>
      ...
      {/* Toast stack — top-centre, just below the Dynamic Island / notch */}
      {toasts.length > 0 && (
        <div
        ...
        >
          {toasts.map((t) => (
            <div
              key={t.id}
              className="max-w-[200px] bg-zinc-800/90 backdrop-blur-sm border border-zinc-700/60 text-zinc-100 text-[13px] leading-tight font-medium px-4 py-2 rounded-full shadow-lg animate-in fade-in slide-in-from-top-2 duration-200 whitespace-nowrap overflow-hidden text-ellipsis"
            >
              {t.message}
            </div>
          ))}
